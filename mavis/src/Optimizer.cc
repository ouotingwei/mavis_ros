    ...
    for(size_t i=0; i<vpMPs.size(); i++)
    {
        ...
        for(map<KeyFrame*,tuple<int,int,int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad())
            {
                ...
                if(pKFi->mpCamera3){
                    int leftsideIndex = get<2>(mit->second);

                    if(leftsideIndex != -1 && leftsideIndex < pKFi->mvKeysLeftSide.size()){
                        leftsideIndex = leftsideIndex - pKFi->NLeft - pKFi->NRight;

                        Eigen::Matrix<double,2,1> obs;
                        kpUn = pKFi->mvKeysLeftSide[leftsideIndex];
                        obs << kpUn.pt.x, kpUn.pt.y;
                ...
                if(pKFi->mpCamera4)
                {
                    int rightsideIndex = get<3>(mit->second);

                    if(rightsideIndex != -1 && rightsideIndex < pKFi->mvKeysRightSide.size()){
                        rightsideIndex = rightsideIndex - pKFi->NLeft - pKFi->NRight - pKFi->NLeftSide;

                        Eigen::Matrix<double,2,1> obs;
                        kpUn = pKFi->mvKeysRightSide[rightsideIndex];
                        obs << kpUn.pt.x, kpUn.pt.y;
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    vector<ORB_SLAM3::EdgeSE3ProjectXYZOnlyLSPoseToBody *> vpEdgesMono_FHLS;
    vector<ORB_SLAM3::EdgeSE3ProjectXYZOnlyRSPoseToBody *> vpEdgesMono_FHRS;

    vector<size_t> vnIndexEdgeMono, vnIndexEdgeRight, vnIndexEdgeLeftSide, vnIndexEdgeRightSide;
    vpEdgesMono.reserve(N);
    vpEdgesMono_FHR.reserve(N);
    vpEdgesMono_FHLS.reserve(N);
    vpEdgesMono_FHRS.reserve(N);

    vnIndexEdgeMono.reserve(N);
    vnIndexEdgeRight.reserve(N);
    vnIndexEdgeLeftSide.reserve(N);
    vnIndexEdgeRightSide.reserve(N);
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            ...
            //SLAM with respect a rigid body
                ...
                else if(i >= pFrame->Nleft + pFrame->Nright && i < pFrame->Nleft + pFrame->Nright + pFrame->Nleftside)
                {
                    kpUn = pFrame->mvKeysLeftSide[i - pFrame->Nleft - pFrame->Nright];
                    ...
                    ORB_SLAM3::EdgeSE3ProjectXYZOnlyLSPoseToBody *e = new ORB_SLAM3::EdgeSE3ProjectXYZOnlyLSPoseToBody();
                    ...
                    e->mTlsl = g2o::SE3Quat(pFrame->GetRelativePoseTlsl().unit_quaternion().cast<double>(), pFrame->GetRelativePoseTlsl().translation().cast<double>());

                    optimizer.addEdge(e);

                    vpEdgesMono_FHLS.push_back(e);
                    vnIndexEdgeLeftSide.push_back(i);
                }
                else {
                    kpUn = pFrame->mvKeysRightSide[i - pFrame->Nleft - pFrame->Nright - pFrame->Nleftside];
                    ...
                    ORB_SLAM3::EdgeSE3ProjectXYZOnlyRSPoseToBody *e = new ORB_SLAM3::EdgeSE3ProjectXYZOnlyRSPoseToBody();
                    ...
                    e->mTrsl = g2o::SE3Quat(pFrame->GetRelativePoseTrsl().unit_quaternion().cast<double>(), pFrame->GetRelativePoseTrsl().translation().cast<double>());

                    optimizer.addEdge(e);

                    vpEdgesMono_FHRS.push_back(e);
                    vnIndexEdgeRightSide.push_back(i);
                }
            }
        }
    }
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        for(size_t i=0, iend=vpEdgesMono_FHLS.size(); i<iend; i++)
        {
            ORB_SLAM3::EdgeSE3ProjectXYZOnlyLSPoseToBody *e = vpEdgesMono_FHLS[i];

            const size_t idx = vnIndexEdgeLeftSide[i];
        ...
        for(size_t i=0, iend=vpEdgesMono_FHRS.size(); i<iend; i++)
        {
            ORB_SLAM3::EdgeSE3ProjectXYZOnlyRSPoseToBody *e = vpEdgesMono_FHRS[i];

            const size_t idx = vnIndexEdgeRightSide[i];
    ...
    if(nInitialCorrespondences-nBad < 15){
        return 0;
    }else{
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
{
    ...
    vector<ORB_SLAM3::EdgeSE3ProjectXYZLSToBody*> vpEdgesMono_LS;
    vpEdgesMono_LS.reserve(nExpectedSize);

    vector<ORB_SLAM3::EdgeSE3ProjectXYZRSToBody*> vpEdgesMono_RS;
    vpEdgesMono_RS.reserve(nExpectedSize);

    vector<KeyFrame*> vpEdgeKFMono_LS;
    vpEdgeKFMono_LS.reserve(nExpectedSize);

    vector<KeyFrame*> vpEdgeKFMono_RS;
    vpEdgeKFMono_RS.reserve(nExpectedSize);

    vector<MapPoint*> vpMapPointEdgeMono_LS;
    vpMapPointEdgeMono_LS.reserve(nExpectedSize);

    vector<MapPoint*> vpMapPointEdgeMono_RS;
    vpMapPointEdgeMono_RS.reserve(nExpectedSize);
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            {
                ...
                if(pKFi->mpCamera3){
                    ...
                    if(leftsideIndex != -1){
                        ...
                        Eigen::Matrix<double,2,1> obs;
                        cv::KeyPoint kp = pKFi->mvKeysLeftSide[leftsideIndex];
                        obs << kp.pt.x, kp.pt.y;

                        ORB_SLAM3::EdgeSE3ProjectXYZLSToBody *e = new ORB_SLAM3::EdgeSE3ProjectXYZLSToBody();
                        ...
                        Sophus::SE3f Tlsl = pKFi->GetRelativePoseTlsl();
                        e->mTlsl = g2o::SE3Quat(Tlsl.unit_quaternion().cast<double>(), Tlsl.translation().cast<double>());

                        e->pCamera = pKFi->mpCamera3;

                        optimizer.addEdge(e);
                        vpEdgesMono_LS.push_back(e);
                        vpEdgeKFMono_LS.push_back(pKFi);
                        vpMapPointEdgeMono_LS.push_back(pMP);
                ...
                if(pKFi->mpCamera4)
                {
                    ...
                    if(rightsideIndex != -1){
                        ...
                        Eigen::Matrix<double,2,1> obs;
                        cv::KeyPoint kp = pKFi->mvKeysRightSide[rightsideIndex];
                        obs << kp.pt.x, kp.pt.y;

                        ORB_SLAM3::EdgeSE3ProjectXYZRSToBody *e = new ORB_SLAM3::EdgeSE3ProjectXYZRSToBody();
                        ...
                        Sophus::SE3f Trsl = pKFi->GetRelativePoseTrsl();
                        e->mTrsl = g2o::SE3Quat(Trsl.unit_quaternion().cast<double>(), Trsl.translation().cast<double>());

                        e->pCamera = pKFi->mpCamera4;

                        optimizer.addEdge(e);
                        vpEdgesMono_RS.push_back(e);
                        vpEdgeKFMono_RS.push_back(pKFi);
                        vpMapPointEdgeMono_RS.push_back(pMP);
    ...
    vToErase.reserve(vpEdgesMono.size()+vpEdgesBody.size()+vpEdgesMono_LS.size()+vpEdgesMono_RS.size()+vpEdgesStereo.size());
    ...
    // Left cam
    for(size_t i=0, iend=vpEdgesMono_LS.size(); i<iend; i++)
    {
        ORB_SLAM3::EdgeSE3ProjectXYZLSToBody *e = vpEdgesMono_LS[i];
        MapPoint *pMP = vpMapPointEdgeMono_LS[i];
        ...
        if(e->chi2()>5.991 || !e->isDepthPositive())
        {
            KeyFrame *pKFi = vpEdgeKFMono_LS[i];
            vToErase.push_back(make_pair(pKFi,pMP));
        }
    }

    // Right cam
    for(size_t i=0, iend = vpEdgesMono_RS.size(); i<iend; i++)
    {
        ORB_SLAM3::EdgeSE3ProjectXYZRSToBody *e = vpEdgesMono_RS[i];
        MapPoint *pMP = vpMapPointEdgeMono_RS[i];
        ...
        if(e->chi2()>5.991 || !e->isDepthPositive())
        {
            KeyFrame *pKFi = vpEdgeKFMono_RS[i];
            vToErase.push_back(make_pair(pKFi,pMP));
        }
    }
            ...
            if(!pKFi->isBad())
            {
                ...
                if(pKFi->mpCamera2){
                    ...
                    if(pKFi->mpCamera2){
                        ...
                        if(rightIndex != -1){
                            rightIndex = rightIndex - pKFi->NLeft;
...
int Optimizer::PoseInertialOptimizationLastKeyFrame(Frame *pFrame, bool bRecInit)
{
    ...
    const int Nleftside = pFrame->Nleftside;
    const int Nrightside = pFrame->Nrightside;
...

    {
        ...
        for(int i=0; i<N; i++)
        {
            ...
            if(pMP)
            {
                ...
                // LeftSide monocular observation
                if(bRight && (i >= Nleft + Nright && i < Nleft + Nright + Nleftside) && (Nleftside != -1 || Nrightside != -1))
                {
                    nInitialMonoCorrespondences++;
                    pFrame->mvbOutlier[i] = false;

                    kpUn = pFrame->mvKeysLeftSide[i - Nleft - Nright];
                    Eigen::Matrix<double,2,1> obs;
                    obs << kpUn.pt.x, kpUn.pt.y;
                ...
                // RightSide monocular observation
                if(bRight && (i >= Nleft + Nright + Nleftside && i < Nleft + Nright + Nleftside + Nrightside) && (Nleftside != -1 || Nrightside != -1))
                {
                    nInitialMonoCorrespondences++;
                    pFrame->mvbOutlier[i] = false;

                    kpUn = pFrame->mvKeysRightSide[i - Nleft - Nright - Nleftside];
                    Eigen::Matrix<double,2,1> obs;
                    obs << kpUn.pt.x, kpUn.pt.y;
