...
Frame::Frame(const Frame &frame)
    :mpcpi(frame.mpcpi),
     ...
     mpORBextractorLeftSide(frame.mpORBextractorLeftSide), mpORBextractorRightSide(frame.mpORBextractorRightSide),
     mbEnableLeft(true), mbEnableRight(true), mbEnableLeftSide(true), mbEnableRightSide(true),
     ...
     mvKeys(frame.mvKeys), mvKeysRight(frame.mvKeysRight), mvKeysLeftSide(frame.mvKeysLeftSide), mvKeysRightSide(frame.mvKeysRightSide), 
     mvKeysUn(frame.mvKeysUn), mvuRight(frame.mvuRight),
     mvDepth(frame.mvDepth), mBowVec(frame.mBowVec), mFeatVec(frame.mFeatVec),
     mDescriptors(frame.mDescriptors.clone()), mDescriptorsRight(frame.mDescriptorsRight.clone()), 
     mDescriptorsLeftSide(frame.mDescriptorsLeftSide.clone()), mDescriptorsRightSide(frame.mDescriptorsRightSide.clone()),
     ...
     Nleft(frame.Nleft), Nright(frame.Nright), Nleftside(frame.Nleftside), Nrightside(frame.Nrightside), 
     monoLeft(frame.monoLeft), monoRight(frame.monoRight), monoLeftSide(frame.monoLeftSide), monoRightSide(frame.monoRightSide),
     ...
     mTlls(frame.mTlls), mRlls(frame.mRlls), mtlls(frame.mtlls), mTlsl(frame.mTlls),
     mTlrs(frame.mTlrs), mRlrs(frame.mRlrs), mtlrs(frame.mtlrs), mTrsl(frame.mTrsl),
     mTcw(frame.mTcw), mbHasPose(false), mbHasVelocity(false)
{
    for(int i=0;i<FRAME_GRID_COLS;i++)
        for(int j=0; j<FRAME_GRID_ROWS; j++) {
            ...
            if(frame.Nleftside > 0 || frame.Nrightside > 0){
                mGridLeftSide[i][j] = frame.mGridLeftSide[i][j];
                mGridRightSide[i][j] = frame.mGridRightSide[i][j];
            }
        }
...
void Frame::AssignFeaturesToGrid()
{
    ...
    for(unsigned int i=0; i<FRAME_GRID_COLS;i++)
        for (unsigned int j=0; j<FRAME_GRID_ROWS;j++){
            ...
            if(Nleftside != -1 || Nrightside != -1){
                mGridLeftSide[i][j].reserve(nReserve);
                mGridRightSide[i][j].reserve(nReserve);
            }
        }

    // N = Nleft | Nright |frontBoundary| NleftSide |leftSideBoundary| NrightSide
    int rightBoundary = Nleft + Nright;
    int leftSideBoundary = Nleft + Nright + Nleftside;
    ...
    for(int i=0;i<N;i++)
    {
        const cv::KeyPoint &kp = (Nleft == -1) ? mvKeysUn[i]
                                    : (i < Nleft) ? mvKeys[i]
                                        : (i < rightBoundary) ? mvKeysRight[i - Nleft]
                                            : (i < leftSideBoundary) ? mvKeysLeftSide[i - rightBoundary]
                                                : mvKeysRightSide[i - leftSideBoundary];
        ...
        if(PosInGrid(kp,nGridPosX,nGridPosY)){
            ...
            else if(i < leftSideBoundary)
            {
                mGridLeftSide[nGridPosX][nGridPosY].push_back(i - rightBoundary);
            }
            else
            {
                mGridRightSide[nGridPosX][nGridPosY].push_back(i - leftSideBoundary);
            }
        }
    }
}
...
void Frame::ExtractORB(int flag, bool enableCamera, const cv::Mat &im, const int x0, const int x1)
{
    ...
    std::vector<cv::KeyPoint> emptyKeys = std::vector<cv::KeyPoint>(0);
    ...
    switch(flag)
    {
        ...
        {
            if(enableCamera){
                monoLeftSide = (*mpORBextractorLeftSide)(im, cv::Mat(), mvKeysLeftSide, mDescriptorsLeftSide, vLapping);
            } else {
                mDescriptorsLeftSide = emptyDescriptors;
                mvKeysLeftSide = emptyKeys;
                monoLeftSide = 0;
            }
            break;
        }
        ...
        {
            if(enableCamera){
                monoRightSide = (*mpORBextractorRightSide)(im, cv::Mat(), mvKeysRightSide, mDescriptorsRightSide, vLapping);
            } else {
                mDescriptorsRightSide = emptyDescriptors;
                mvKeysRightSide = emptyKeys;
                monoRightSide = 0;
            }
            break;
        }
...
// left to leftside poses
Sophus::SE3f Frame::GetRelativePoseTlsl(){
    return mTlsl;
}

Sophus::SE3f Frame::GetRelativePoseTlls(){
    return mTlls;
}

Eigen::Matrix3f Frame::GetRelativePoseTlls_rotation(){
    return mTlls.rotationMatrix();
}

Eigen::Vector3f Frame::GetRelativePoseTlls_translation(){
    return mTlls.translation();
}

// front cam to right side cam
Sophus::SE3f Frame::GetRelativePoseTrsl(){
    return mTrsl;
}

Sophus::SE3f Frame::GetRelativePoseTlrs(){
    return mTlrs;
}

Eigen::Matrix3f Frame::GEtRelativePoseTlrs_rotation(){
    return mTlrs.rotationMatrix();
}

Eigen::Vector3f Frame::GetRelativePoseTlrs_translation(){
    return mTlrs.translation();
}

// must modified this part
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit)
{
    // initialize the state of the map points
    pMP->mbTrackInView = false;
    pMP->mbTrackInViewR = false;
    pMP->mbTrackInViewLS = false;
    pMP->mbTrackInViewRS = false;

    pMP->mnTrackScaleLevel = -1;
    pMP->mnTrackScaleLevelR = -1;
    pMP->mnTrackScaleLevelLS = -1;
    pMP->mnTrackScaleLevelRS = -1;
    ...
    else
    {
        ...
        else
        {
            pMP->mbTrackInView = isInFrustumChecks(pMP,viewingCosLimit,0);
            pMP->mbTrackInViewR = isInFrustumChecks(pMP,viewingCosLimit,1);
            pMP->mbTrackInViewLS = isInFrustumChecks(pMP,viewingCosLimit,2);
            pMP->mbTrackInViewRS = isInFrustumChecks(pMP,viewingCosLimit,3);
            return (pMP->mbTrackInView || pMP->mbTrackInViewR || pMP->mbTrackInViewLS || pMP->mbTrackInViewRS);
        }
    }
}
...
vector<size_t> Frame::GetFeaturesInArea(const float &x, const float  &y, const float  &r, const int minLevel, const int maxLevel, const int Camera) const
{
    ...
    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)
    {
        ...
        {
            const vector<size_t> vCell = (Camera == 0) ? mGrid[ix][iy]
                                        : (Camera == 1) ? mGridRight[ix][iy]
                                            : (Camera == 2) ? mGridLeftSide[ix][iy]
                                                : (Camera == 3) ? mGridRightSide[ix][iy]
            ...
            for(size_t j=0, jend=vCell.size(); j<jend; j++)
            {
                const cv::KeyPoint &kpUn = (Nleft == -1) ? mvKeysUn[vCell[j]]
                                           : (Camera == 0) ? mvKeys[vCell[j]]
                                                : (Camera == 1) ? mvKeysRight[vCell[j]]
                                                    : (Camera == 2) ? mvKeysLeftSide[vCell[j]]
                                                        : (Camera == 3) ? mvKeysRightSide[vCell[j]]
                                                            : mvKeysUn[vCell[j]];
                if(bCheckLevels)
                {
...
{
    // speed it up by matching keypoints in the overlapping area
    vector<cv::KeyPoint> stereoLeft(mvKeys.begin() + monoLeft, mvKeys.end());
    vector<cv::KeyPoint> stereoRight(mvKeysRight.begin() + monoRight, mvKeysRight.end());
...
// must modified this part
bool Frame::isInFrustumChecks(MapPoint *pMP, float viewingCosLimit, int Camera)
{
    // 3D in absolute coordinates
    Eigen::Vector3f P = pMP->GetWorldPos();
    ...
    switch(Camera)
    {
        ...
        {
            // front cam
            mR = mRcw;      // cw = world frame 2 camera frame
            mt = mtcw;      // Ow = the camera's position in the world coordinate system
            twc = mOw;
        } break;
        case 1:
        {
            // front cam (stereo right)
        ...
        {
            // left cam
            Eigen::Matrix3f Rlsl = mTlsl.rotationMatrix();
            Eigen::Vector3f tlsl = mTlsl.translation();
            mR = Rlsl * mRcw;
            mt = Rlsl * mtcw + tlsl;
            twc = mRwc * mTlls.translation() + mOw;
        } break;
        case 3:
        {
            Eigen::Matrix3f Rrsl = mTrsl.rotationMatrix();
            Eigen::Vector3f trsl = mTrsl.translation();
            mR = Rrsl * mRcw;
            mt = Rrsl * mtcw + trsl;
            twc = mRwc * mTlrs.translation() + mOw;
        } break;
    }
    ...
    switch(Camera)
    {
        case 0:
        {
            uv = mpCamera->project(Pc); // check project
            break;
        }
        case 1:
        {
    ...
    switch(Camera)
    {
        ...
        case 2:
        {
            pMP->mTrackProjXLS = uv(0);
            pMP->mTrackProjYLS = uv(1);
            pMP->mnTrackScaleLevelLS = nPredictedLevel;
            pMP->mTrackViewCosLS = viewCos;
            pMP->mTrackDepthLS = Pc_dist;
        } break;
        case 3:
        {
            pMP->mTrackProjXRS = uv(0);
            pMP->mTrackProjYRS = uv(1);
            pMP->mnTrackScaleLevelRS = nPredictedLevel;
            pMP->mTrackViewCosRS = viewCos;
            pMP->mTrackDepthRS = Pc_dist;
        } break;
    }
...
// MCSLAM+DE
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const cv::Mat &imLeftSide, const cv::Mat &imRightSide, const double &timeStamp, 
                ORBextractor* extractorLeft, ORBextractor* extractorRight, ORBextractor* ORBextractorLeftSide, ORBextractor* ORBextractorRightSide, 
                bool bleft, bool bright, bool bleftside, bool brightside, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, 
                GeometricCamera* pCamera, GeometricCamera* pCamera2, GeometricCamera* pCamera3, GeometricCamera* pCamera4, 
                Sophus::SE3f& Tlr, Sophus::SE3f& Tlls, Sophus::SE3f& Tlrs, Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL), mpORBvocabulary(voc),mpORBextractorLeft(extractorLeft),mpORBextractorRight(extractorRight), mpORBextractorLeftSide(ORBextractorLeftSide), mpORBextractorRightSide(ORBextractorRightSide),
     mbEnableLeft(bleft), mbEnableRight(bright), mbEnableLeftSide(bleftside), mbEnableRightSide(brightside),
     mTimeStamp(timeStamp), mK(K.clone()), mK_(Converter::toMatrix3f(K)), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
     ...
     mpCamera(pCamera), mpCamera2(pCamera2), mpCamera3(pCamera3) ,mpCamera4(pCamera4), mbHasPose(false), mbHasVelocity(false)
{
    //clone image
    imgLeft = imLeft.clone();
    imgRight = imRight.clone();
    imgLeftSide = imLeftSide.clone();
    imgRightSide = imRightSide.clone();
    ...
    thread threadLeftSide(&Frame::ExtractORB,this,2,bleftside,imLeftSide,0,0);
    thread threadRightSide(&Frame::ExtractORB,this,3,brightside,imRightSide,0,0);
    threadLeft.join();
    threadRight.join();
    threadLeftSide.join();
    threadRightSide.join();
#ifdef REGISTER_TIMES
    std::chrono::steady_clock::time_point time_EndExtORB = std::chrono::steady_clock::now();
    ...
    Nleft = mvKeys.size();
    Nright = mvKeysRight.size();
    Nleftside = mvKeysLeftSide.size();
    Nrightside = mvKeysRightSide.size();
    N = Nleft+Nright+Nleftside+Nrightside;

    // std::cout << "[DEBUG] Feature counts:" << std::endl;
    // std::cout << " - Left         : " << mvKeys.size() << std::endl;
    // std::cout << " - Right        : " << mvKeysRight.size() << std::endl;
    // std::cout << " - LeftSide     : " << mvKeysLeftSide.size() << std::endl;
    // std::cout << " - RightSide    : " << mvKeysRightSide.size() << std::endl;
    ...
    mTlls = Tlls;
    mTlsl = mTlls.inverse();
    mRlls = mTlls.rotationMatrix();
    mtlls = mTlls.translation();

    mTlrs = Tlrs;
    mTrsl = mTlrs.inverse();
    mRlrs = mTlrs.rotationMatrix();
    mtlrs = mTlrs.translation();
    ...
    cv::vconcat(mDescriptorsLeftSide, mDescriptorsRightSide,sideDescriptors);
