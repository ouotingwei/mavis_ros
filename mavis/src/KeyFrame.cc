...
KeyFrame::KeyFrame():
        ...
        NLeft(0),NRight(0), NLeftSide(0), NRightSide(0), mnNumberOfOpt(0), mbHasVelocity(false)
{
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mTlr(F.GetRelativePoseTlr()), mTrl(F.GetRelativePoseTrl()), mTlls(F.GetRelativePoseTlls()), mTlsl(F.GetRelativePoseTlsl()), mTlrs(F.GetRelativePoseTlrs()), mTrsl(F.GetRelativePoseTrsl()),
    mvKeysRight(F.mvKeysRight), mvKeysLeftSide(F.mvKeysLeftSide), mvKeysRightSide(F.mvKeysRightSide),
    NLeft(F.Nleft), NRight(F.Nright), NLeftSide(F.Nleftside), NRightSide(F.Nrightside), mnNumberOfOpt(0), mbHasVelocity(false)
{
    mnId=nNextId++;
    ...
    if(F.Nleftside != -1 || F.Nrightside != -1) {
        mGridLeftSide.resize(mnGridCols);
        mGridRightSide.resize(mnGridCols);
    }
    ...
    for(int i=0; i<mnGridCols;i++)
    {
        ...
        if(F.Nleftside != -1 || F.Nrightside != -1) {
            mGridLeftSide[i].resize(mnGridRows);
            mGridRightSide[i].resize(mnGridRows);
        }
        ...
        for(int j=0; j<mnGridRows; j++){
            ...
            if(F.Nleftside != -1 || F.Nrightside != -1){
                mGridLeftSide[i][j] = F.mGridLeftSide[i][j];
                mGridRightSide[i][j] = F.mGridRightSide[i][j];
            }
        }
    }
...
Sophus::SE3f KeyFrame::GetPose(const int cameraID)
{
    ...
    return (cameraID == 0) ? mTcw :
                (cameraID == 1) ? mTrl * mTcw :
                    (cameraID == 2) ? mTlsl * mTcw :
                        (cameraID == 3) ? mTrsl * mTcw
...
Sophus::SE3f KeyFrame::GetPoseInverse(const int cameraID)
{
    ...
    return (cameraID == 0) ? mTwc :
                (cameraID == 1) ? mTwc * mTlr :
                    (cameraID == 2) ? mTwc * mTlls :
                        (cameraID == 3) ? mTwc * mTlrs
                                        : throw std::invalid_argument("Invalid camera Idx: " + std::to_string(cameraID));
}
...
Eigen::Matrix3f KeyFrame::GetRotation(const int cameraID){
    ...
    return (cameraID == 0) ? mRcw
                : (cameraID == 1) ? (mTrl.so3() * mTcw.so3()).matrix()
                    :(cameraID == 2) ? (mTlsl.so3() * mTcw.so3()).matrix()
                        : (cameraID == 3) ? (mTrsl.so3() * mTcw.so3()).matrix()
                                        : throw std::invalid_argument("Invalid camera Idx: " + std::to_string(cameraID));
}
...
Eigen::Vector3f KeyFrame::GetTranslation(const int cameraID)
{
    ...
    return (cameraID == 0) ? mTcw.translation() 
                : (cameraID == 1) ? (mTrl * mTcw).translation() 
                    : (cameraID == 2) ? (mTlsl * mTcw).translation() 
                        : (cameraID == 3) ? (mTrsl * mTcw).translation()
                                        : throw std::invalid_argument("Invalid camera Idx: " + std::to_string(cameraID));
}
...
void KeyFrame::EraseMapPointMatch(MapPoint* pMP)
{
    ...
    size_t leftIndex = get<0>(indexes), rightIndex = get<1>(indexes), leftsideIndex = get<2>(indexes), rightsideIndex = get<3>(indexes);
    if(leftIndex != -1)
        mvpMapPoints[leftIndex]=static_cast<MapPoint*>(NULL);
    if(rightIndex != -1)
        mvpMapPoints[rightIndex]=static_cast<MapPoint*>(NULL);
    if(leftsideIndex != -1)
        mvpMapPoints[leftsideIndex]=static_cast<MapPoint*>(NULL);
    if(rightsideIndex != -1)
        mvpMapPoints[rightsideIndex]=static_cast<MapPoint*>(NULL);
}
    ...
    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)
    {
        for(int iy = nMinCellY; iy<=nMaxCellY; iy++)
        {
            const vector<size_t> vCell = (cameraID == 0) ? mGrid[ix][iy]  
                                        ...
                                        : (cameraID == 2) ? mGridLeftSide[ix][iy] 
                                        : (cameraID == 3) ? mGridRightSide[ix][iy]
                                        : throw std::invalid_argument("Invalid camera Idx: " + std::to_string(cameraID));
            ...
            for(size_t j=0, jend=vCell.size(); j<jend; j++)
            {
                const cv::KeyPoint &kpUn = (NLeft == -1) ? mvKeysUn[vCell[j]]
                                        ...
                                        : (cameraID == 2) ? mvKeysLeftSide[vCell[j]]
                                        : (cameraID == 3) ? mvKeysRightSide[vCell[j]]
                                        : throw std::invalid_argument("Invalid camera Idx: " + std::to_string(cameraID));
                ...
                if(fabs(distx)<r && fabs(disty)<r)
                    vIndices.push_back( (cameraID == 0) ? vCell[j]
                                        ...
                                        : (cameraID == 3) ? vCell[j] + NLeft + NRight + NLeftSide
                                        : throw std::invalid_argument("Invalid camera Idx: " + std::to_string(cameraID)));
            }
        }
    }
...
Sophus::SE3f KeyFrame::GetRelativePoseTlls()
{
    unique_lock<mutex> lock(mMutexPose);
    return mTlls;
}

Sophus::SE3<float> KeyFrame::GetLeftSidePose()
{
    unique_lock<mutex> lock(mMutexPose);
    return mTlsl * mTcw;
}

Sophus::SE3<float> KeyFrame::GetLeftSidePoseInverse()
{
    unique_lock<mutex> lock(mMutexPose);
    return mTwc * mTlls;
}

Eigen::Vector3f KeyFrame::GetLeftSideCameraCenter()
{
    unique_lock<mutex> lock(mMutexPose);
    return (mTwc * mTlls).translation();
}

Eigen::Matrix<float,3,3> KeyFrame::GetLeftSideRotation()
{
    unique_lock<mutex> lock(mMutexPose);
    return (mTlsl.so3() * mTcw.so3()).matrix();
}

Eigen::Vector3f KeyFrame::GetLeftSideTranslation()
{
    unique_lock<mutex> lock(mMutexPose);
    return (mTlsl * mTcw).translation();
}

// Right Side
Sophus::SE3f KeyFrame::GetRelativePoseTrsl()
{
    unique_lock<mutex> lock(mMutexPose);
    return mTrsl;
}

Sophus::SE3f KeyFrame::GetRelativePoseTlrs()
{
    unique_lock<mutex> lock(mMutexPose);
    return mTlrs;
}

Sophus::SE3<float> KeyFrame::GetRightSidePose()
{
    unique_lock<mutex> lock(mMutexPose);
    return mTrsl * mTcw;
}

Sophus::SE3<float> KeyFrame::GetRightSidePoseInverse()
{
    unique_lock<mutex> lock(mMutexPose);
    return mTwc * mTlrs;
}

Eigen::Vector3f KeyFrame::GetRightSideCameraCenter()
{
    unique_lock<mutex> lock(mMutexPose);
    return (mTwc * mTlrs).translation();
}

Eigen::Matrix<float,3,3> KeyFrame::GetRightSideRotation()
{
    unique_lock<mutex> lock(mMutexPose);
    return (mTrsl.so3() * mTcw.so3()).matrix();
}

Eigen::Vector3f KeyFrame::GetRightSideTranslation()
{
    unique_lock<mutex> lock(mMutexPose);
    return (mTrsl * mTcw).translation();
}
...
const cv::KeyPoint &KeyFrame::GetKey(const int kpIdx) const
{
    ...
    switch(cameraId)
    {
        ...
        case 2:
            return mvKeysLeftSide[kpIdx - NLeft - NRight];
        case 3:
            return mvKeysRightSide[kpIdx - NLeft - NRight - NLeftSide];
        default:
    ...
    return (NLeft == -1) ? 0
                : (kpIdx < NLeft) ? 0
                    : (kpIdx < NLeft + NRight) ? 1
                        : (kpIdx < NLeft + NRight + NLeftSide) ? 2
                            : 3;
}
