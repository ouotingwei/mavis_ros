    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(bFindedRegion && mbActiveLC)
            {
                if(mbMergeDetected)
                {
                    if ((mpTracker->mSensor==System::IMU_MONOCULAR || mpTracker->mSensor==System::IMU_STEREO || mpTracker->mSensor==System::IMU_RGBD || mpTracker->mSensor==System::MULTI) &&
                    ...
                    else
                    {
                        ...
                        switch(mMergeCameraPair.cameraID1)
                        {
                            ...
                            case 2:
                            {
                                auto Tlls = mpCurrentKF->GetRelativePoseTlls();
                                g2o::Sim3 Slls(Tlls.unit_quaternion().cast<double>(), Tlls.translation().cast<double>(), 1.0);
                                gSw2c = (Slls * mg2oMergeSlw).inverse();
                            } break;

                            case 3:
                            {
                                auto Tlrs = mpCurrentKF->GetRelativePoseTlrs();
                                g2o::Sim3 Slrs(Tlrs.unit_quaternion().cast<double>(), Tlrs.translation().cast<double>(), 1.0);
                                gSw2c = (Slrs * mg2oMergeSlw).inverse();
                            } break;
                        }
                                                 ...
                                                 std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs, CameraMatch &mCameraPair)
{
    ...
    if(nNumProjMatches >= nProjMatches)
    {
        ...
        if(mpTracker->mSensor==System::IMU_MONOCULAR && !pCurrentKF->GetMap()->GetIniertialBA2())
            bFixedScale=false;
        int numOptMatches = Optimizer::OptimizeSim3(mpCurrentKF, pMatchedKF, vpMatchedMPs, gScm, 10, bFixedScale, mHessian7x7, true, mCameraPair.cameraID1, mCameraPair.cameraID2);
                                             ...
                                             int &nNumCoincidences, std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs, CameraMatch &mCameraPair)
{
    ...
    ORBmatcher matcherBoW(0.9, false);      // TODO: Why ?
