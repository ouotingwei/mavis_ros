...
namespace ORB_SLAM3 {
    ...
    bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false) {
        ...
        if(sensor_ == System::STEREO || sensor_ == System::IMU_STEREO || sensor_ == System::MULTI){
            readCamera2(fSettings);
            cout << "\t-Loaded camera 2" << endl;
        }

        if(sensor_ == System::MULTI){
            readCamera3(fSettings);
            readCamera4(fSettings);
            cout << "\t-Loaded camera left(3) and right(4)" << endl;
        }
        ...
        if(sensor_ == System::IMU_MONOCULAR || sensor_ == System::IMU_STEREO || sensor_ == System::IMU_RGBD || sensor_ == System::MULTI){
            readIMU(fSettings);
    ...
    void Settings::readImageInfo(cv::FileStorage &fSettings) {
        ...
        if(found){
            ...
            if(!bNeedToRectify_){
                ...
                if((sensor_ == System::STEREO || sensor_ == System::IMU_STEREO || sensor_ == System::MULTI) && cameraType_ != Rectified){
    ...
    void Settings::readIMU(cv::FileStorage &fSettings) {
        ...
        if(sensor_ == System::MULTI){
            ...
            Tlls_ = Tbc_.inverse() * Tbcl_;
            Tlrs_ = Tbc_.inverse() * Tbcr_;
            Tls_ = Tlls_.inverse();
            Trs_ = Tlrs_.inverse() * Tlr_;
        }
    ...
    void Settings::precomputeRectificationMaps() {
        ...
        if(sensor_ == System::IMU_STEREO || sensor_ == System::MULTI){
    ...
    ostream &operator<<(std::ostream& output, const Settings& settings){
        ...
        if (settings.sensor_ == System::STEREO || settings.sensor_ == System::IMU_STEREO || settings.sensor_ == System::MULTI)
        {
        ...
        if(settings.sensor_ == System::MULTI)
        {
            output << "\t-Camera 3(Left) parameters (";
            if(settings.cameraType_ == Settings::PinHole || settings.cameraType_ == Settings::Rectified){
                output << "Pinhole";
            }
            else{
            ...
            output << "\t-Camera 4(Right) parameters (";
            if(settings.cameraType_ == Settings::PinHole || settings.cameraType_ == Settings::Rectified){
                output << "Pinhole";
            }
            else{
        ...
        if(settings.bNeedToRectify_){
            ...
            // if(settings.sensor_ == System::STEREO || settings.sensor_ == System::IMU_STEREO || setting.sensor_ == System::MULTI){
