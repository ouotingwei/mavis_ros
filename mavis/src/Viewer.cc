...
void Viewer::Run()
{
    ...
    pangolin::Var<bool> menuEnableLeftSideCam("menu.Enable LeftSide Cam",true,true);
    pangolin::Var<bool> menuEnableRightSideCam("menu.Enable RightSide Cam",true,true);
    ...
    Ow.SetIdentity();
    cv::namedWindow("MC-SLAM: Current Frame");
    ...
    while(1)
    {
        ...
        if(menuEnableLeftSideCam)
            mpTracker->mbleftside = true;
        else
            mpTracker->mbleftside = false;
        
        if(menuEnableRightSideCam)
            mpTracker->mbrightside = true;
        else
            mpTracker->mbrightside = false;
        ...
        else if(mpTracker->mSensor==mpSystem->MULTI){
            cv::Mat frontImgs, sideImgs;
            cv::Mat imRight = mpFrameDrawer->DrawRightFrame(trackedImageScale);
            cv::Mat imLeftSide = mpFrameDrawer->DrawLeftSideFrame(trackedImageScale);
            cv::Mat imRightSide = mpFrameDrawer->DrawRightSideFrame(trackedImageScale);
            cv::hconcat(im,imRight,frontImgs);
            cv::hconcat(imLeftSide,imRightSide,sideImgs);
            cv::hconcat(frontImgs,sideImgs,toShow);
        }
        else{
            toShow = im;
        }
        ...
        cv::imshow("MC-SLAM: Current Frame",toShow);
        cv::waitKey(mT);
