...
namespace ORB_SLAM3 {
    ...
    // MCSLAM+DE Left Camera
    bool EdgeSE3ProjectXYZOnlyLSPoseToBody::read(std::istream& is) {
        for(int i=0; i<2; i++) {
            is >> _measurement[i];
        }
    ...
    // MCSLAM+DE Left Camera
    bool EdgeSE3ProjectXYZOnlyLSPoseToBody::write(std::ostream& os) const {
        for(int i=0; i<2; i++) {
            os << measurement()[i] << " ";
        }
    ...
    // MCSLAM+DE Left Camera
    void EdgeSE3ProjectXYZOnlyLSPoseToBody::linearizeOplus() {
        g2o::VertexSE3Expmap * vi = static_cast<g2o::VertexSE3Expmap *>(_vertices[0]);
        g2o::SE3Quat T_lw(vi->estimate());
        Eigen::Vector3d X_l = T_lw.map(Xw);
        Eigen::Vector3d X_ls = mTlsl.map(T_lw.map(Xw));
        ...
        _jacobianOplusXi = -pCamera->projectJac(X_ls) * mTlsl.rotation().toRotationMatrix() * SE3deriv;
    }

    // MCSLAM+DE Right Camera
    bool EdgeSE3ProjectXYZOnlyRSPoseToBody::read(std::istream& is) {
        for(int i=0; i<2; i++) {
            is >> _measurement[i];
        }
    ...
    // MCSLAM+DE Right Camera
    bool EdgeSE3ProjectXYZOnlyRSPoseToBody::write(std::ostream& os) const {
        for(int i=0; i<2; i++) {
            os << measurement()[i] << " ";
        }
    ...
    // MCSLAM+DE Right Camera
    void EdgeSE3ProjectXYZOnlyRSPoseToBody::linearizeOplus() {
        g2o::VertexSE3Expmap * vi = static_cast<g2o::VertexSE3Expmap *>(_vertices[0]);
        g2o::SE3Quat T_lw(vi->estimate());
        Eigen::Vector3d X_l = T_lw.map(Xw);
        Eigen::Vector3d X_rs = mTrsl.map(T_lw.map(Xw));
        ...
        _jacobianOplusXi = -pCamera->projectJac(X_rs) * mTrsl.rotation().toRotationMatrix() * SE3deriv;
    }
    ...
    // MCSLAM+DE Left Camera
    EdgeSE3ProjectXYZLSToBody::EdgeSE3ProjectXYZLSToBody() : BaseBinaryEdge<2, Eigen::Vector2d, g2o::VertexSBAPointXYZ, g2o::VertexSE3Expmap>() {
    }

    // MCSLAM+DE Left Camera
    bool EdgeSE3ProjectXYZLSToBody::read(std::istream& is){
        for (int i=0; i<2; i++){
            is >> _measurement[i];
        }
        for (int i=0; i<2; i++)
    ...
    // MCSLAM+DE Left Camera
    bool EdgeSE3ProjectXYZLSToBody::write(std::ostream& os) const {
    ...
    // MCSLAM+DE Left Camera
    void EdgeSE3ProjectXYZLSToBody::linearizeOplus() {
        g2o::VertexSE3Expmap * vj = static_cast<g2o::VertexSE3Expmap *>(_vertices[1]);
        g2o::SE3Quat T_lw(vj->estimate());
        g2o::SE3Quat T_lsw = mTlsl * T_lw;
        g2o::VertexSBAPointXYZ* vi = static_cast<g2o::VertexSBAPointXYZ*>(_vertices[0]);
        Eigen::Vector3d X_w = vi->estimate();
        Eigen::Vector3d X_l = T_lw.map(X_w);
        Eigen::Vector3d X_ls = mTlsl.map(T_lw.map(X_w));

        _jacobianOplusXi =  -pCamera->projectJac(X_ls) * T_lsw.rotation().toRotationMatrix();
        ...
        _jacobianOplusXj = -pCamera->projectJac(X_ls) * mTlsl.rotation().toRotationMatrix() * SE3deriv;
    }

    // MCSLAM+DE Right Camera
    EdgeSE3ProjectXYZRSToBody::EdgeSE3ProjectXYZRSToBody() : BaseBinaryEdge<2, Eigen::Vector2d, g2o::VertexSBAPointXYZ, g2o::VertexSE3Expmap>() {
    }

    // MCSLAM+DE Right Camera
    bool EdgeSE3ProjectXYZRSToBody::read(std::istream& is){
        for (int i=0; i<2; i++){
            is >> _measurement[i];
        }
        for (int i=0; i<2; i++)
    ...
    // MCSLAM+DE Right Camera
    bool EdgeSE3ProjectXYZRSToBody::write(std::ostream& os) const {
    ...
    // MCSLAM+DE Right Camera
    void EdgeSE3ProjectXYZRSToBody::linearizeOplus() {
        g2o::VertexSE3Expmap * vj = static_cast<g2o::VertexSE3Expmap *>(_vertices[1]);
        g2o::SE3Quat T_lw(vj->estimate());
        g2o::SE3Quat T_rsw = mTrsl * T_lw;
        g2o::VertexSBAPointXYZ* vi = static_cast<g2o::VertexSBAPointXYZ*>(_vertices[0]);
        Eigen::Vector3d X_w = vi->estimate();
        Eigen::Vector3d X_l = T_lw.map(X_w);
        Eigen::Vector3d X_rs = mTrsl.map(T_lw.map(X_w));

        _jacobianOplusXi =  -pCamera->projectJac(X_rs) * T_rsw.rotation().toRotationMatrix();
        ...
        _jacobianOplusXj = -pCamera->projectJac(X_rs) * mTrsl.rotation().toRotationMatrix() * SE3deriv;
    }
