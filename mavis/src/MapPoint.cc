    ...
    mnOriginMapId(pMap->GetId())
{
    ...
    mbTrackInView = false;
    mbTrackInViewLS = false;
    mbTrackInViewRS = false;
    ...
    mnFound(1), mbBad(false), mpReplaced(NULL), mpMap(pMap), mnOriginMapId(pMap->GetId())
{
    ...
    else if(idxF >= pFrame->Nleft + pFrame->Nright && idxF < pFrame->Nleft + pFrame->Nright + pFrame->Nleftside)
    {
        Eigen::Matrix3f Rwl = pFrame->GetRwc();
        Eigen::Vector3f tlls = pFrame->GetRelativePoseTlls().translation();
        Eigen::Vector3f twl = pFrame->GetOw();
        ...
        Eigen::Matrix3f Rwl = pFrame->GetRwc();
        Eigen::Vector3f tlrs = pFrame->GetRelativePoseTlrs().translation();
        Eigen::Vector3f twl = pFrame->GetOw();
    ...
    const int level = (pFrame->Nleft == -1) ? pFrame->mvKeysUn[idxF].octave
                    ...
                    : (idxF < pFrame->Nleft + pFrame->Nright + pFrame->Nleftside) ? pFrame->mvKeysLeftSide[idxF].octave
                    : pFrame->mvKeysRightSide[idxF].octave;
...
void MapPoint::AddObservation(KeyFrame* pKF, int idx)
{
    ...
    if(pKF->NLeft == -1) get<0>(indexes) = idx;
    else if(idx < pKF->NLeft) get<0>(indexes) = idx;
    else if(idx >= pKF->NLeft && idx < pKF->NLeft + pKF->NRight) get<1>(indexes) = idx;
    else if(idx >= pKF->NLeft + pKF->NRight && idx < pKF->NLeft + pKF->NRight + pKF->NLeftSide) get<2>(indexes) = idx;
    else get<3>(indexes) = idx;
...
void MapPoint::EraseObservation(KeyFrame* pKF)
{
    bool bBad=false;
    {
        ...
        if(mObservations.count(pKF))
        {
            ...
            int leftIndex = get<0>(indexes), rightIndex = get<1>(indexes), leftsideIndex = get<2>(indexes), rightsideIndex = get<3>(indexes);
...
void MapPoint::SetBadFlag()
{
    ...
    for(map<KeyFrame*, tuple<int,int,int,int>>::iterator mit=obs.begin(), mend=obs.end(); mit!=mend; mit++)
    {
        ...
        int leftIndex = get<0>(mit -> second), rightIndex = get<1>(mit -> second), leftsideIndex = get<2>(mit->second), rightsideIndex = get<3>(mit->second);
        if(leftIndex != -1){
        ...
        if(leftsideIndex != -1){
            pKF->EraseMapPointMatch(leftsideIndex);
        }
        if(rightsideIndex != -1){
            pKF->EraseMapPointMatch(rightsideIndex);
        }
    }
...
void MapPoint::Replace(MapPoint* pMP)
{
    ...
    for(map<KeyFrame*,tuple<int,int,int,int>>::iterator mit=obs.begin(), mend=obs.end(); mit!=mend; mit++)
    {
        ...
        if(!pMP->IsInKeyFrame(pKF))
        {
            ...
            if(leftsideIndex != -1){
                pKF->ReplaceMapPointMatch(leftsideIndex, pMP);
                pMP->AddObservation(pKF,leftsideIndex);
            }
            if(rightsideIndex != -1){
                pKF->ReplaceMapPointMatch(rightsideIndex, pMP);
                pMP->AddObservation(pKF,rightsideIndex);
            }
        }
        else
        {
...
void MapPoint::ComputeDistinctiveDescriptors()
{
    ...
    for(map<KeyFrame*,tuple<int,int,int,int>>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
    {
        ...
        if(!pKF->isBad()){
            ...
            if(leftsideIndex != -1){
                vDescriptors.push_back(pKF->mDescriptors.row(leftsideIndex));
            }
            if(rightsideIndex != -1){
                vDescriptors.push_back(pKF->mDescriptors.row(rightsideIndex));
            }
        }
    }
...
void MapPoint::UpdateNormalAndDepth()
{
    ...
    for(map<KeyFrame*,tuple<int,int,int,int>>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
    {
        ...
        if(leftsideIndex != -1){
            Eigen::Vector3f Owi = pKF->GetLeftSideCameraCenter();
            Eigen::Vector3f normali = Pos - Owi;
            normal = normal + normali / normali.norm();
            n++;
        }
        if(rightsideIndex != -1){
            Eigen::Vector3f Owi = pKF->GetRightSideCameraCenter();
            Eigen::Vector3f normali = Pos - Owi;
            normal = normal + normali / normali.norm();
            n++;
        }
    ...
    if(pRefKF -> NLeft == -1){
        ...
        level = pRefKF -> mvKeysLeftSide[leftsideIndex - pRefKF -> NLeft - pRefKF -> NRight].octave;
    }
    else{
        level = pRefKF -> mvKeysRightSide[rightsideIndex - pRefKF -> NLeft - pRefKF -> NRight - pRefKF -> NLeftSide].octave;
    }
...
void MapPoint::PreSave(set<KeyFrame*>& spKF,set<MapPoint*>& spMP)
{
    ...
    // std::map<KeyFrame*,std::tuple<int,int,int,int> > tmp_mObservations;
...
int MapPoint::GetTrackScaleLevel(const int cameraID) const {
    return (cameraID == 0) ? mnTrackScaleLevel
                            ...
                            : (cameraID == 2) ? mnTrackScaleLevelLS
                            : (cameraID == 3) ? mnTrackScaleLevelRS
