    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    if (mSensor==IMU_STEREO || mSensor==IMU_MONOCULAR || mSensor==IMU_RGBD || mSensor==MULTI)
    ...
    mpLocalMapper = new LocalMapping(this, mpAtlas, mSensor==MONOCULAR || mSensor==IMU_MONOCULAR,
                                     mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO || mSensor==IMU_RGBD || mSensor==MULTI, strSequence);
...
Sophus::SE3f System::TrackMC(const cv::Mat &imLeft, const cv::Mat &imRight, const cv::Mat &imLeftSide, const cv::Mat&imRightSide, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    if(mSensor!= MULTI)
    {
        cerr << "ERROR: you called TrackMC but input sensor was not set to Multi-Camera." << endl;
        exit(-1);
    }
 
    cv::Mat imLeftToFeed, imRightToFeed, imLeftSideToFeed, imRightSideToFeed;
    if(settings_ && settings_->needToRectify()){
        cv::Mat M1l = settings_->M1l();
        cv::Mat M2l = settings_->M2l();
        cv::Mat M1r = settings_->M1r();
        ...
        cv::resize(imLeft,imLeftToFeed,settings_->newImSize());
        cv::resize(imRight,imRightToFeed,settings_->newImSize());
        cv::resize(imLeftSide, imLeftSideToFeed, settings_->newImSize());
        cv::resize(imRightSide, imRightSideToFeed, settings_->newImSize());
    }
    else{
        imLeftToFeed = imLeft.clone();
        imRightToFeed = imRight.clone();
        imLeftSideToFeed = imLeftSide.clone();
        imRightSideToFeed = imRightSide.clone();
    }
    ...
    Sophus::SE3f Twb = mpTracker->GrabImageMC(imLeftToFeed, imRightToFeed, imLeftSideToFeed, imRightSideToFeed, timestamp, filename);
...
void System::SaveTrajectoryEuRoC(const string &filename)
{
    ...
    if (mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO || mSensor==IMU_RGBD || mSensor==MULTI)
