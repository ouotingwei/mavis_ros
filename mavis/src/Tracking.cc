    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL))
{
    ...
    if(settings){
        ...
        if(sensor==System::IMU_MONOCULAR || sensor==System::IMU_STEREO || sensor==System::IMU_RGBD || sensor==System::MULTI)
        {
...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    if(mSensor==System::STEREO || mSensor==System::IMU_STEREO || mSensor==System::IMU_RGBD || mSensor==System::MULTI){
        // rgbd-imu mode will triger this, but mpCamera2 is unused, only setting IMU
    ...
    if(mSensor==System::STEREO || mSensor==System::RGBD || mSensor==System::IMU_STEREO || mSensor==System::IMU_RGBD || mSensor==System::MULTI){
        mbf = settings->bf();
    ...
    // must modified the setting part
    if(mSensor==System::MULTI) {
        ...
        mTlls = settings->Tlls(); // left(front) -> left side camera
        mTlrs = settings->Tlrs (); // left(right) -> right side camera
    }
    ...
    if(mSensor==System::STEREO || mSensor==System::IMU_STEREO || mSensor==System::MULTI)
    ...
    // for Multi-camera
    if(mSensor==System::MULTI) {
        mpORBextractorLeftSide = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
        mpORBextractorRightSide = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
    }
...
// MC-SLAM
Sophus::SE3f Tracking::GrabImageMC(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const cv::Mat &imRectLeftSide, const cv::Mat &imRectRightSide, const double &timestamp, string filename)
{
    //cout << "GrabImageStereo" << endl;

    mImGray = imRectLeft;
    cv::Mat imGrayRight = imRectRight;
    cv::Mat imGrayLeftSide = imRectLeftSide;
    cv::Mat imGrayRightSide = imRectRightSide;
    mImLeft = imRectLeft;
    mImRight = imRectRight;
    mImLeftSide = imRectLeftSide; // -> not define yet
    mImRightSide = imRectRightSide;
    ...
    if(mImGray.channels()==3)
    {
        ...
        if(mbRGB)
        {
            cvtColor(mImGray,mImGray,cv::COLOR_RGB2GRAY);
            cvtColor(imGrayRight,imGrayRight,cv::COLOR_RGB2GRAY);
            cvtColor(imGrayLeftSide, imGrayLeftSide, cv::COLOR_RGB2GRAY);
            cvtColor(imGrayRightSide, imGrayRightSide, cv::COLOR_RGB2GRAY);
        }
        else
        {
            cvtColor(mImGray,mImGray,cv::COLOR_BGR2GRAY);
            cvtColor(imGrayRight,imGrayRight,cv::COLOR_BGR2GRAY);
            cvtColor(imGrayLeftSide, imGrayLeftSide, cv::COLOR_BGR2GRAY);
            cvtColor(imGrayRightSide, imGrayRightSide, cv::COLOR_BGR2GRAY);
    ...
    else if(mImGray.channels()==4)
    {
        ...
        if(mbRGB)
        {
            cvtColor(mImGray,mImGray,cv::COLOR_RGBA2GRAY);
            cvtColor(imGrayRight,imGrayRight,cv::COLOR_RGBA2GRAY);
            cvtColor(imGrayLeftSide, imGrayLeftSide, cv::COLOR_RGBA2GRAY);
            cvtColor(imGrayRightSide, imGrayRightSide, cv::COLOR_RGBA2GRAY);
        }
        else
        {
            cvtColor(mImGray,mImGray,cv::COLOR_BGRA2GRAY);
            cvtColor(imGrayRight,imGrayRight,cv::COLOR_BGRA2GRAY);
            cvtColor(imGrayLeftSide, imGrayLeftSide, cv::COLOR_BGRA2GRAY);
            cvtColor(imGrayRightSide, imGrayRightSide, cv::COLOR_BGRA2GRAY);
        }
    }

    if(mSensor == System::MULTI)
        mCurrentFrame = Frame(mImGray, imGrayRight, imGrayLeftSide, imGrayRightSide, timestamp, 
                                mpORBextractorLeft, mpORBextractorRight, mpORBextractorLeftSide, mpORBextractorRightSide, mbleft, mbright, mbleftside, mbrightside, 
                                mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpCamera, mpCamera2, mpCamera3, mpCamera4, mTlr, mTlls, mTlrs, &mLastFrame, *mpImuCalib);
...
void Tracking::Track()
{
    ...
    // If the LocalMapping thread detects an issue with the IMU, reset the current map.
    ...
    // Extract the active map from atlas
    ...
    if(mState!=NO_IMAGES_YET)
    {
        ...
        {
            // pre-img time > current-img time -> error
    ...
    if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD || mSensor == System::MULTI) && mpLastKeyFrame)
    ...
    if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD || mSensor == System::MULTI) && !mbCreatedMap)
    {
#ifdef REGISTER_TIMES
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                if (!bOK)
                {
                    if ( mCurrentFrame.mnId<=(mnLastRelocFrameId+mnFramesToResetIMU) &&
                         (mSensor==System::IMU_MONOCULAR || mSensor==System::IMU_STEREO || mSensor == System::IMU_RGBD || mSensor == System::MULTI))
                    {
                        mState = LOST;
                    }
            ...
            else
            {
                ...
                if (mState == RECENTLY_LOST)
                {
                    ...
                    bOK = true;
                    if(mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD || mSensor == System::MULTI)
                    {
        ...
        if((mCurrentFrame.mnId<(mnLastRelocFrameId+mnFramesToResetIMU)) && (mCurrentFrame.mnId > mnFramesToResetIMU) &&
           (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD || mSensor == System::MULTI) && pCurrentMap->isImuInitialized())
        {
...
void Tracking::CreateMapInAtlas()
{
    ...
    if (mSensor==System::IMU_STEREO || mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_RGBD || mSensor == System::MULTI)
    ...
    if((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD || mSensor == System::MULTI) && mpImuPreintegratedFromLastKF)
    {
        delete mpImuPreintegratedFromLastKF;
        mpImuPreintegratedFromLastKF = new IMU::Preintegrated(IMU::Bias(),*mpImuCalib);
    }
    ...
    for(int i =0; i<mCurrentFrame.N; i++)
    {
        ...
        if(mCurrentFrame.mvpMapPoints[i])
        {
            if(mCurrentFrame.mvbOutlier[i])
            {
                ...
                else if(i < mCurrentFrame.Nleft+mCurrentFrame.Nright+mCurrentFrame.Nleftside && i >= mCurrentFrame.Nleft+mCurrentFrame.Nright){
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // std::cout << "[Search by Projection]: " << nmatches << std::endl;
...
bool Tracking::TrackLocalMap()
{
    ...
    else if (mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD || mSensor==System::MULTI)
    {
...
{
    if((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD || mSensor == System::MULTI) && !mpAtlas->GetCurrentMap()->isImuInitialized())
    {
        // if t_current - t_last_keyframe >= 0.25 sec -> need new keyframe
        if (mSensor == System::IMU_MONOCULAR && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=0.25)
            return true;
        else if ((mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD || mSensor == System::MULTI) && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=0.25)
            return true;
        else
            return false;
    }
    ...
    const bool c1c = mSensor!=System::MONOCULAR && mSensor!=System::IMU_MONOCULAR && mSensor!=System::IMU_STEREO && mSensor!=System::IMU_RGBD && mSensor!=System::MULTI && (mnMatchesInliers<nRefMatches*0.25 || bNeedToInsertClose) ;
...
void Tracking::SearchLocalPoints()
{
    ...
    if(nToMatch>0)
    {
        ...
        else if(!mpAtlas->isImuInitialized() && (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD || mSensor == System::MULTI))
        {
            th=10;
        }
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    if((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD || mSensor == System::MULTI) &&mvpLocalKeyFrames.size()<80)
    {
