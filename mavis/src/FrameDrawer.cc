...
FrameDrawer::FrameDrawer(Atlas* pAtlas, const int sensor):both(false),mpAtlas(pAtlas), mSensor(sensor)
{
    ...
    mImLeftSide = cv::Mat(480,640,CV_8UC3, cv::Scalar(0,0,0));
    mImRightSide = cv::Mat(480,640,CV_8UC3, cv::Scalar(0,0,0));
}
...
cv::Mat FrameDrawer::DrawFrame(float imageScale)
{
    ...
    // for analysis
    // 取得當前時間（Unix timestamp，秒+毫秒）
    auto now = std::chrono::system_clock::now();
    auto now_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
        now.time_since_epoch()).count();
    std::cout << "[MatchRate-Left] "
          << "t=" << now_ms << " "
          << "Match to a MapPoint: " << mnTracked << std::endl;
...
cv::Mat FrameDrawer::DrawRightFrame(float imageScale)
{
    ...
    auto now_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
        now.time_since_epoch()).count();
    std::cout << "[MatchRate-Right] "     << "t=" << now_ms << " " << "Match to a MapPoint: " << mnTracked  << std::endl;
...
cv::Mat FrameDrawer::DrawLeftSideFrame(float imageScale)
{
    cv::Mat im;
    vector<cv::KeyPoint> vIniKeys; // Initialization: KeyPoints in reference frame
    vector<int> vMatches; // Initialization: correspondeces with reference keypoints
    ...
    //Copy variables within scoped mutex
    {
        ...
        mImLeftSide.copyTo(im);

        if(mState==Tracking::NOT_INITIALIZED)
        {
            vCurrentKeys = mvCurrentKeysLeftSide;
            vIniKeys = mvIniKeys;
            vMatches = mvIniMatches;
        }
        else if(mState==Tracking::OK)
    ...
    else if(state==Tracking::OK) //TRACKING
    {
        ...
        mnTrackedVO=0;
        const float r = 5;
        const int n = mvCurrentKeysLeftSide.size();
        ...
        for(int i=0;i<n;i++)
        {
            if(vbVO[i + RightBoundary] || vbMap[i + RightBoundary])
            {
                ...
                if(imageScale != 1.f)
                {
                    point = mvCurrentKeysLeftSide[i].pt / imageScale;
                    float px = mvCurrentKeysLeftSide[i].pt.x / imageScale;
                    float py = mvCurrentKeysLeftSide[i].pt.y / imageScale;
                    pt1.x=px-r;
                    pt1.y=py-r;
                    pt2.x=px+r;
                    pt2.y=py+r;
                ...
                {
                    point = mvCurrentKeysLeftSide[i].pt;
                    pt1.x=mvCurrentKeysLeftSide[i].pt.x-r;
                    pt1.y=mvCurrentKeysLeftSide[i].pt.y-r;
                    pt2.x=mvCurrentKeysLeftSide[i].pt.x+r;
                    pt2.y=mvCurrentKeysLeftSide[i].pt.y+r;
                }
    ...
    auto now_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
        now.time_since_epoch()).count();
    std::cout << "[MatchRate-LeftSide] "  << "t=" << now_ms << " " << "Match to a MapPoint: " << mnTracked  << std::endl;
...
cv::Mat FrameDrawer::DrawRightSideFrame(float imageScale)
{
    cv::Mat im;
    vector<cv::KeyPoint> vIniKeys; // Initialization: KeyPoints in reference frame
    vector<int> vMatches; // Initialization: correspondeces with reference keypoints
    ...
    //Copy variables within scoped mutex
    {
        ...
        mImRightSide.copyTo(im);

        if(mState==Tracking::NOT_INITIALIZED)
        {
            vCurrentKeys = mvCurrentKeysRightSide;
            vIniKeys = mvIniKeys;
            vMatches = mvIniMatches;
        }
        else if(mState==Tracking::OK)
    ...
    else if(state==Tracking::OK) //TRACKING
    {
        ...
        mnTrackedVO=0;
        const float r = 5;
        const int n = mvCurrentKeysRightSide.size();
        const int LeftSideBoundary = mvCurrentKeys.size() + mvCurrentKeysRight.size() + mvCurrentKeysLeftSide.size();

        // cout << "[DEBUG] RightSide size = " << n << ", vbVO size = " << vbVO.size()
        // << ", vbMap size = " << vbMap.size()
        // << ", LeftSideBoundary = " << LeftSideBoundary << endl;


        for(int i=0;i<n;i++)
        {
            if(vbVO[i + LeftSideBoundary] || vbMap[i + LeftSideBoundary])
            {
                cv::Point2f pt1,pt2;
                cv::Point2f point;
                if(imageScale != 1.f)
                {
                    point = mvCurrentKeysRightSide[i].pt / imageScale;
                    float px = mvCurrentKeysRightSide[i].pt.x / imageScale;
                    float py = mvCurrentKeysRightSide[i].pt.y / imageScale;
                    pt1.x=px-r;
                    pt1.y=py-r;
                    pt2.x=px+r;
                    pt2.y=py+r;
                ...
                {
                    point = mvCurrentKeysRightSide[i].pt;
                    pt1.x=mvCurrentKeysRightSide[i].pt.x-r;
                    pt1.y=mvCurrentKeysRightSide[i].pt.y-r;
                    pt2.x=mvCurrentKeysRightSide[i].pt.x+r;
                    pt2.y=mvCurrentKeysRightSide[i].pt.y+r;
                }

                // This is a match to a MapPoint in the map
                if(vbMap[i + LeftSideBoundary])
                {
                    cv::rectangle(im,pt1,pt2,cv::Scalar(0,255,0));
                    cv::circle(im,point,2,cv::Scalar(0,255,0),-1);
                    mnTracked++;
    ...
    auto now_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
        now.time_since_epoch()).count();
    std::cout << "[MatchRate-RightSide] " << "t=" << now_ms << " " << "Match to a MapPoint: " << mnTracked << std::endl;
...
void FrameDrawer::DrawTextInfo(cv::Mat &im, int nState, cv::Mat &imText)
{
    ...
    imText = cv::Mat(im.rows+textSize.height+20,im.cols,im.type());
    im.copyTo(imText.rowRange(0,im.rows).colRange(0,im.cols));
    imText.rowRange(im.rows,imText.rows) = cv::Mat::zeros(textSize.height+20,im.cols,im.type());
    cv::putText(imText,s.str(),cv::Point(5,imText.rows-5),cv::FONT_HERSHEY_PLAIN,1.5,cv::Scalar(255,255,255),2,cv::LINE_AA);
...
void FrameDrawer::Update(Tracking *pTracker)
{
    ...
    if((mSensor==System::STEREO || mSensor==System::IMU_STEREO || mSensor==System::IMU_RGBD) && both) {
        ...
        mvCurrentKeysRight = pTracker->mCurrentFrame.mvKeysRight;
        mvCurrentKeysLeftSide = pTracker->mCurrentFrame.mvKeysLeftSide;
        mvCurrentKeysRightSide = pTracker->mCurrentFrame.mvKeysRightSide;
        pTracker->mImRight.copyTo(mImRight);
        pTracker->mImLeftSide.copyTo(mImLeftSide);
        pTracker->mImRightSide.copyTo(mImRightSide);
        N = mvCurrentKeys.size() + mvCurrentKeysRight.size() + mvCurrentKeysLeftSide.size() + mvCurrentKeysRightSide.size();
    }
    else {
