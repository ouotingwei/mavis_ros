...
public:
    ...
    Sophus::SE3f GetRelativePoseTlls();
    Eigen::Matrix3f GetRelativePoseTlls_rotation();
    Eigen::Vector3f GetRelativePoseTlls_translation();

    // MCSLAM+DE right
    Sophus::SE3f GetRelativePoseTrsl();
    Sophus::SE3f GetRelativePoseTlrs();
    Eigen::Matrix3f GEtRelativePoseTlrs_rotation();
    Eigen::Vector3f GetRelativePoseTlrs_translation();
    ...
    vector<size_t> GetFeaturesInArea(const float &x, const float  &y, const float  &r, const int minLevel=-1, const int maxLevel=-1, const int Camera=0) const;
...
private:
    ...
    // MCSLAM+DE left
    Sophus::SE3<float> mTlls, mTlsl;
    Sophus::Matrix<float,3,3> mRlls;
    Eigen::Vector3f mtlls;

    // MCSLAM+DE right
    Sophus::SE3<float> mTlrs, mTrsl;
    Sophus::Matrix<float,3,3> mRlrs;
    Eigen::Vector3f mtlrs;
...
public:
    ...
    // MCSLAM+DE
    ORBextractor* mpORBextractorLeftSide;
    ORBextractor* mpORBextractorRightSide;

    // MCSLAM+DE
    bool mbEnableLeft, mbEnableRight, mbEnableLeftSide, mbEnableRightSide;
    ...
    // MCSLAM+DE
    std::vector<cv::KeyPoint> mvKeysLeftSide, mvKeysRightSide;
    ...
    // MCSLAM+DE
    cv::Mat mDescriptorsLeftSide, mDescriptorsRightSide;
...
public:
    ...
    // MCSLAM_DE
    GeometricCamera* mpCamera3;
    GeometricCamera* mpCamera4;

    //Number of KeyPoints extracted in the left and right images
    int Nleft, Nright;

    //Number of KeyPoints extracted in the leftside and rightside images
    int Nleftside, Nrightside;

    //Number of Non Lapping Keypoints
    int monoLeft, monoRight, monoLeftSide, monoRightSide;
    ...
    // MCSLAM+DE left
    std::vector<std::size_t> mGridLeftSide[FRAME_GRID_COLS][FRAME_GRID_ROWS];
    // MCSLAM+DE right
    std::vector<std::size_t> mGridRightSide[FRAME_GRID_COLS][FRAME_GRID_ROWS];
    ...
    // Constructor for MCSLAM+DE
    Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const cv::Mat &imLeftSide, const cv::Mat &imRightSide, const double &timeStamp, 
                ORBextractor* extractorLeft, ORBextractor* extractorRight, ORBextractor* extractorLeftSide, ORBextractor* extractorRightSide, 
                bool bEnableLeft, bool bEnableRight, bool bEnableLeftSide, bool bEnableRightSide, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, 
                GeometricCamera* pCamera, GeometricCamera* pCamera2, GeometricCamera* pCamera3, GeometricCamera* pCamera4, 
                Sophus::SE3f& Tlr, Sophus::SE3f& Tlls, Sophus::SE3f& Tlrs, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
    ...
    bool isInFrustumChecks(MapPoint* pMP, float viewingCosLimit, int Camera=0);

    Eigen::Vector3f UnprojectStereoFishEye(const int &i);

    cv::Mat imgLeft, imgRight;
    // MCSLAM+DE -> ?
    cv::Mat imgRightSide, imgLeftSide;
