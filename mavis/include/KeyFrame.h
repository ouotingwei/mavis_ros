...
class KeyFrame
{
    ...
    void serialize(Archive& ar, const unsigned int version)
    {
        ...
        serializeVectorKeyPoints<Archive>(ar, mvKeysLeftSide, version);
        serializeVectorKeyPoints<Archive>(ar, mvKeysRightSide, version);
        ...
        ar & const_cast<int&>(NLeftSide);
        ar & const_cast<int&>(NRightSide);
        serializeSophusSE3<Archive>(ar, mTlr, version);
        serializeSophusSE3<Archive>(ar, mTrl, version);
        serializeSophusSE3<Archive>(ar, mTlls, version);
        serializeSophusSE3<Archive>(ar, mTlrs, version);
        serializeSophusSE3<Archive>(ar, mTlsl, version);
        serializeSophusSE3<Archive>(ar, mTrsl, version);
        serializeVectorKeyPoints<Archive>(ar, mvKeysRight, version);
        serializeVectorKeyPoints<Archive>(ar, mvKeysLeftSide, version);
        serializeVectorKeyPoints<Archive>(ar, mvKeysRightSide, version);
        ar & mGridRight;
...
protected:
    ...
    //Transformation matrix between cameras in MC-SLAM
    Sophus::SE3<float> mTlls, mTlrs;
    Sophus::SE3<float> mTlsl, mTrsl;
    ...
    // Backup for Cameras
    unsigned int mnBackupIdCamera, mnBackupIdCamera2, mnBackupIdCamera3, mnBackupIdCamera4;
...
public:
    GeometricCamera* mpCamera, *mpCamera2, *mpCamera3, *mpCamera4;
    ...
    const std::vector<cv::KeyPoint> mvKeysRight, mvKeysLeftSide, mvKeysRightSide;

    const int NLeft, NRight, NLeftSide, NRightSide;

    std::vector< std::vector <std::vector<size_t> > > mGridRight, mGridLeftSide, mGridRightSide;

    Sophus::SE3<float> GetRightPose();
    Sophus::SE3<float> GetRightPoseInverse();

    Sophus::SE3<float> GetLeftSidePose();
    Sophus::SE3<float> GetLeftSidePoseInverse();

    Sophus::SE3<float> GetRightSidePose();
    Sophus::SE3<float> GetRightSidePoseInverse();

    Eigen::Vector3f GetRightCameraCenter();
    Eigen::Matrix<float,3,3> GetRightRotation();
    Eigen::Vector3f GetRightTranslation();

    Eigen::Vector3f GetLeftSideCameraCenter();
    Eigen::Matrix<float,3,3> GetLeftSideRotation();
    Eigen::Vector3f GetLeftSideTranslation();

    Eigen::Vector3f GetRightSideCameraCenter();
    Eigen::Matrix<float,3,3> GetRightSideRotation();
    Eigen::Vector3f GetRightSideTranslation();
